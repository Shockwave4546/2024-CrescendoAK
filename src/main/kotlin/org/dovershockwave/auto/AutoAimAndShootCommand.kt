...
class AutoAimAndShootCommand(intake: IntakeSubsystem, shooter: ShooterSubsystem, arm: IntakeArmSubsystem, wrist: ShooterWristSubsystem, swerve: SwerveSubsystem, vision: VisionSubsystem) : SequentialCommandGroup() {
  init {
    addCommands(
      ConditionalCommand(TrackSpeakerCommand(swerve, vision), InstantCommand()) { AimAndShootCommand.AUTO_AIM_ON_SHOOT.get() },
      AutoFullShootInterpolatedCommand(intake, shooter, arm, wrist)
    )
